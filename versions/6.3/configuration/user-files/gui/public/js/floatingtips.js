...
var FloatingTips = new Class({
	...
	_create: function(elem) {
		...
		var paddTop = Math.abs(this.options.distance) + 'px';
		var tip = new Element('div').addClass(o.className + '-wrapper').setStyles({ 'margin': 0, 'padding': 0, 'padding-top': paddTop, 'z-index': cwr.getStyle('z-index') }).adopt(cwr);
		...
		if (o.balloon && !Browser.ie6) {
			...
			trg = new Element('div').addClass(o.className + '-triangle').setStyles({ 'margin': 0, 'padding': 0 });
			var trgSt = { 'border-color': cwr.getStyle('border-color'), 'border-width': o.arrowSize, 'border-style': 'solid','width': 0, 'height': 0 };

			switch (opos) {
				case 'inside':
				case 'top': trgSt['border-bottom-width'] = 0; break;
				case 'right': trgSt['border-left-width'] = 0; trg.addClass(o.className + '-triangle-right'); trgSt['float'] = 'left'; cwr.setStyle('margin-left', o.arrowSize + 5); break;
		...
		// fix top position in case of persistent headers are set
		pos.y += $('main-container').getStyle('margin-top').toInt();
		...
		// fix right margin
		if (o.html) {
			if ($(document.body).getSize().x - 10 < tip.getSize().x + tip.getPosition().x) {
				var originalLeft = tip.getStyle('left').toInt();
				tip.setStyle('left', $(document.body).getSize().x - tip.getSize().x);
				var currentLeft = tip.getStyle('left').toInt();	
				trg.setStyle('margin-left', trg.getStyle('margin-left').toInt() + originalLeft - currentLeft + "px");
			}
		}
		
		if ($(document.body).getSize().x - 10 < tip.getSize().x + tip.getPosition().x) {
			var size = tip.getSize().x + tip.getPosition().x - $(document.body).getSize().x;
			tip.setStyle('width', tip.getSize().x - size + 'px')
			tip.setStyle('word-wrap', 'wordwrap');
			
			tip.setStyle('left', tip.getStyle('left').toInt() + window.pageXOffset + 'px');
			trg.setStyle('margin-left', trg.getStyle('margin-left').toInt() - window.pageXOffset + 'px');
		}
	...
	_animate: function(tip, d) {
		...
		$$('.floating-tip').each(function(item) {
			var totalSize = item.getPosition().x + item.getSize().x;
			if (totalSize > $(document.body).getSize().x) {
				var diff = $(document.body).getSize().x - totalSize;
				item.setStyle('width', item.getSize().x.toInt() + diff - 15);
			}
		});
